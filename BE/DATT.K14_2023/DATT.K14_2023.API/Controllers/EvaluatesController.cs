...
namespace DATT.K14_2023.API.Controllers
{
    ...
    public class EvaluatesController : ControllerBase
    {
            ...
            )
        {
            try
            {
                dynamic records = _evaluateBL.GetRecordByFilterAndPaging(pageSize, pageNumber, keyWord, status);
        ...
        [HttpPut("{id}")]
        public IActionResult UpdateRecord(Guid id, [FromBody] Evaluate record)
        {
            try
            {
                var result = _evaluateBL.UpdateRecord(id, record);
                if (result.IsSuccess)
                {
                    return StatusCode(StatusCodes.Status201Created);
                }
